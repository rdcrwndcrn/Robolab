...
def following_line():
    ...
    def wait_for_colour_calibration():
        while not ts.value():
            time.sleep(0.01)

    wait_for_colour_calibration()
    ...
    while not ts.value():
        # preparing colour sensor
        # using rgb-mode
        cs.mode = 'RGB-RAW'
        # us the value in a variable
        c = cs.raw
        ...
        light_grey = 0.3 * cs.value(0) + 0.59 * cs.value(1) + 0.11 * cs.value(2)
